...
## קטלוג — Google Drive הוא מקור האמת
תיקיית השורש: **📸 קטלוג בלונים**. כל תת-תיקייה בה היא קטגוריה באפליקציה,
ותמונות שיושבות ישירות בשורש מוצגות תחת "כללי".

- תמונה שתושם ידנית בתיקייה בדרייב תופיע באפליקציה ברענון לשונית הקטלוג.
- העלאה מהאפליקציה נכתבת לתיקייה שנבחרה בדרייב, עם שם הקובץ המקורי.
- מחיקה מעבירה לאשפה בדרייב (`trashed:true`), לא מוחקת לצמיתות.
- הרשת מציגה `thumbnailLink` ב-s800; שיתוף מוריד את הקובץ המלא דרך `alt=media`.
- וידאו בתיקיות מסונן החוצה — הקטלוג מציג תמונות בלבד.

⚠️ **ה-scope הוא `auth/drive` מלא.** `drive.file` לא מספיק: הוא נותן גישה רק
לקבצים שהאפליקציה עצמה יצרה, ולכן לא היה רואה אף תמונה קיימת.
