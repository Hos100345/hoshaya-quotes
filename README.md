...
## מחירון
`PL` ב-`index.html` הוא העתק של הגיליון **"מחירון הושעיה אמן בלונים 2026"** בדרייב
(38 פריטים, 6 קטגוריות כולל **חנות**). הגיליון הוא מקור האמת — שינוי שם או מחיר
מתחיל שם ומועתק לקוד באותו סבב, אחרת ההצעה ללקוח והמחירון מתפצלים.

- `b` מחיר רגיל · `l` מחיר תושב יישוב (רק כשהוא שונה) · `u` יחידה · `c` קטגוריה.
- `MIN_CALLOUT` ו-`TRAVEL_RATE` הם קבועים — לא לפזר 1.5 בקוד. היו שלושה חישובי
  נסיעות נפרדים (מסך, הודעת וואטסאפ, דרישת תשלום), וכל שינוי בתעריף היה מפצל אותם.
